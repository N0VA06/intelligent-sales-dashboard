...
const SalesKnowledgeGraph: React.FC = () => {
  // Map our custom nodes to ReactFlow nodes
  const initialNodes: Node[] = knowledgeGraphNodes.map((node: KnowledgeGraphNode) => ({
    id: node.id,
    type: node.type,
  ...
  const initialEdges: Edge[] = knowledgeGraphEdges.map((edge: KnowledgeGraphEdge) => ({
    id: edge.id,
    source: edge.source,
    target: edge.target,
    type: 'custom',
  ...
  return (
    <div className="w-full h-full knowledge-graph">
      <ReactFlow
        nodes={filteredNodes}
        edges={filteredEdges}
